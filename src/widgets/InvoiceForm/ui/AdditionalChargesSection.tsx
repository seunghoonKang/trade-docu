      ...
      {charges.map((charge, i) => (
        <div key={i} className="flex gap-3 items-end">
          <div className="flex-1">
            <input className="w-full px-3 py-2 text-base border border-border rounded" placeholder={t("form.description")} value={charge.description} onChange={(e) => onUpdateCharge(i, "description", e.target.value)} />
          </div>
          <div className="w-32">
            <input className="w-full px-3 py-2 text-base border border-border rounded text-right" type="number" min="0" step="0.01" placeholder={t("form.amount")} value={charge.amount || ""} onChange={(e) => onUpdateCharge(i, "amount", Number(e.target.value))} />
          </div>
